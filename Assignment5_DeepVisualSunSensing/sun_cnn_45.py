...
class CNN(torch.nn.Module):
    def __init__(self, num_bins): 
        ...
        ## CONVOLUTION LAYERS
        self.conv1 = torch.nn.Conv2d(3, 16, stride=4, padding = 1, kernel_size=(9,9))
        self.conv2 = torch.nn.Conv2d(16, 16, stride=1, padding = 1, kernel_size=(3,3))
        self.conv3 = torch.nn.Conv2d(16,num_bins, kernel_size=(1,6))
        ## POOLING LAYERS
...
class dataloader(torch.utils.data.Dataset):
    ...
    def normalize_to_zero_mean(self):
        #---FILL ME IN-----
        batchMean = self.data['images'].mean(axis = (2,3), keepdims = True) 
        batchSTD = self.data['images'].std(axis = (2,3), keepdims = True)
        self.data['images'] = (self.data['images'] - batchMean)/batchSTD
        #------------------
        return

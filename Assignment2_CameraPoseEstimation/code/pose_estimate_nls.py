...
def pose_estimate_nls(K, Twc_guess, Ipts, Wpts):
    ...
    psHat = np.zeros((2*tp, 1))
    
    # setup K_tilde to compute
    K_tilde = np.zeros((4,4))
    K_tilde[:3,:3] = K
    K_tilde[3][3] = 1
    
    # obtain the T_cw matrix
    Twc = np.zeros((4,4))
    T_wc_curr = Twc_guess
    T_cw = inv(Twc_guess)
    ...
    currWpt = np.zeros((4,1))
    currCpt = np.zeros((4,1))
    currJ = np.zeros((2,6))
    JTpose = None
    J_LInv = None
    qResiduals = None
    ...
    while True:
        ...
        for i in np.arange(tp):
            ...
            # compute the projection on the plane
            currWpt =  np.vstack((np.reshape(Wpts[:,i], (3,1)), np.array([1])))
            currCpt = np.dot(K_tilde, np.dot(T_cw, currWpt))
            psHat[2*i] = currCpt[0]/currCpt[2]
            psHat[2*i + 1] = currCpt[1]/currCpt[2]
            
            # compute Jacobian at world point
            currJ = find_jacobian(K, T_wc_curr, np.reshape(Wpts[:,i], (3,1)))
            J[2*i, :] = currJ[0, :]
            J[2*i + 1, :] = currJ[1, :]
        ...
        dY = ps - psHat
        JTpose = J.T
        J_LInv = inv(np.dot(JTpose, J))
        qResiduals = np.dot(J_LInv, np.dot(JTpose, dY))
        params = params_prev + qResiduals
        ...
        T_wc_curr = hpose_from_epose(params)
        T_cw = inv(T_wc_curr)
